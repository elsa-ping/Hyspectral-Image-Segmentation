...
def extract_test_patches():
  ...
  row_max = 2544          # row_max=2544  7x7
  col_max = 7194         # col_max=7194
  count = 0
  """
  for row_x in enumerate(tqdm(range(row_max),desc='Generating test patches!!!')):
    # compute row number
  ...
  """
  test_datas = np.zeros(shape=[row_max*col_max,8,7,7],dtype=np.int16)
...
def extract_margin_test_patches():     
  ...
  for idx,row in enumerate(range(row_max)):
    ...
    np.savez(os.path.join(datadir2,'col_%d.npz'%(idx+1)),image=patch,coord=(row,-1))
    
  for idx,col in enumerate(range(col_max)):
    x1,x2 = 7*col,7*(col+1)
    patch = row_margin[...,x1:x2]
    np.savez(os.path.join(datadir2,'row_%d.npz'%(idx+1)),image=patch,coord=(-1,col))
