...
def postprocess(num=1):
  ...
  for path in tqdm(margin_paths,desc='Switch Margin'):
    ...
    for (x1,y1),value in zip(coord,predict_value):
      # row margin
      if x1==-1:
        row_margins[:,y1*32:(y1+1)*32] = value
      else:
      # col margin
        col_margins[x1*32:(x1+1)*32,:] = value
        
  # right down little patch
  patch = np.ones(shape=[18,26],dtype=np.int16)*row_margins[1,-1]
