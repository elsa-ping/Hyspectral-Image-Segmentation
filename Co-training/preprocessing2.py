...
def extract_train_patches():
  ...
  datadir1 = './dataset_1/train/train1'
  datadir2 = './dataset_1/train/train2'
  ...
  for idx,info in enumerate(tqdm(infos,desc='Generating Train Patches')):
    ...
    # crop patch
    x1,x2 = center_x-16,center_x+16   # crop 32x32  original 7x7
    y1,y2 = abs(center_y)-16,abs(center_y)+16
    patch = image[:,y1:y2,x1:x2]
    if idx%2==0:
      np.savez(os.path.join(datadir1,'true_%d.npz'%(idx+1)),image=patch,label=label)
    else:
      np.savez(os.path.join(datadir2,'true_%d.npz'%(idx+1)),image=patch,label=label)
...
def extract_test_patches():
  ...
  image = io.imread('./image.tif')
  # save every patch
  datadir1 = './dataset_1/test/region'
  
  if not os.path.exists(datadir1):
    os.makedirs(datadir1)
  row_max = 556          # row_max=2544  7x7
  col_max = 1573         # col_max=7194
  count = 0
  test_datas = np.zeros(shape=[row_max*col_max,8,32,32],dtype=np.int16)
  ...
  for row_x in tqdm(range(row_max),desc='Generating test patches!!!'):
    ...
    for col_y in range(col_max):
      ...
      count += 1
  np.savez('./test_dataset.npz',data=test_datas,coord=test_coords)
  

def extract_margin_test_patches():     
  # simple save margin pixels
  datadir2 = './dataset_1/test/margin'
  if not os.path.exists(datadir2):
    os.makedirs(datadir2)
    
  image = io.imread('./image.tif')
  
  col_margin = image[:,:-18,-32:]
