...
def postprocess(num=1):
  ...
  rows = 2544
  cols = 7194
  filenames = os.listdir('./results/region_1')
  filepaths = [os.path.join('./results/region_1',name) for name in filenames]
  ...
  margin_names = os.listdir('./results/margin_1')
  margin_paths = [os.path.join('./results/margin_1',name) for name in margin_names]
  ...
  # fusion train pixel
  train_infos = np.load('./train_patches.npy').astype(np.int16)
